...
class KernelClient:
    ...
    def execute(self, code, timeout=REQUEST_TIMEOUT):
        ...
        return decode_cellmsg(response)
    ...
    def _get_response(self, msg_id, timeout, post_idle):
        ...
        self.log.debug(f"response_queues: {self.response_queues}")
...
class AccuKernelClient(KernelClient):
    def __init__(
            self, http_api_endpoint, ws_api_endpoint, kernel_id, timeout=REQUEST_TIMEOUT, logger=None
    ):
        super().__init__(http_api_endpoint, ws_api_endpoint, kernel_id, timeout=timeout, logger=logger)
        self.post_idle = None
    ...
    def run(self, code) -> str:
        ...
        try:
            msg_id = self._send_request(code)
            self.post_idle = False
            return msg_id
        except Exception as e:
            self.log.error(e)
            raise e

    def pull_response(self, msg_id: str, timeout: int = REQUEST_TIMEOUT) -> Optional[str]:
        """Response Result"""
        response = []

        try:
            response_message = self._get_response(msg_id, timeout, self.post_idle)
            self.log.debug(f"response_message: {response_message}")

            if response_message:
                response_message_type = response_message["msg_type"]

                if response_message_type == "error" or (
                        response_message_type == "execute_reply" and response_message["content"]["status"] == "error"
                ):
                    response.extend(
                        [
                            '{} : {}'.format(
                    ...
                    self.log.debug(
                        "Unhandled response for msg_id: {} of msg_type: {}".format(msg_id, response_message_type)
                    )
        except Exception as e:
            self.log.error(e)
            raise e
